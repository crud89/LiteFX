...
constexpr UniquePtr<VulkanShaderModule> VulkanShaderProgramBuilder::makeShaderModule(ShaderStage type, const String& fileName, const String& entryPoint, std::optional<UInt32> index)
{
    return makeUnique<VulkanShaderModule>(m_impl->m_device, type, fileName, entryPoint, index.value_or(m_impl->findModuleIndex(type)));
}

constexpr UniquePtr<VulkanShaderModule> VulkanShaderProgramBuilder::makeShaderModule(ShaderStage type, std::istream& stream, const String& name, const String& entryPoint, std::optional<UInt32> index)
{
    return makeUnique<VulkanShaderModule>(m_impl->m_device, type, stream, name, entryPoint, index.value_or(m_impl->findModuleIndex(type)));
}
