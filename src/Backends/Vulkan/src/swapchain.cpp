...
public:
	VkSwapchainKHR initialize(const Format& format, const Size2d& renderArea, const UInt32& buffers)
	{
		...
		m_presentImages = imageChain |
			std::views::transform([this, &actualRenderArea, &selectedFormat](const VkImage& image) { return makeUnique<VulkanImage>(*m_parent->getDevice(), image, Size3d{ actualRenderArea.width(), actualRenderArea.height(), 1 }, selectedFormat, ImageDimensions::DIM_2, 1, 1, false, ResourceState::Undefined); }) |
