...
public:
	VkSwapchainKHR initialize(const Format& format, const Size2d& renderArea, const UInt32& buffers)
	{
		...
		m_presentImages = imageChain |
			std::views::transform([this, &actualRenderArea, &selectedFormat](const VkImage& image) { return makeUnique<VulkanImage>(*m_parent->getDevice(), image, actualRenderArea, selectedFormat, ImageDimensions::DIM_2); }) |
