    ...
    public:
        VulkanQueue& createQueue(const VulkanDevice& device, QueuePriority priority) {
            ...
            if (left == std::end(m_queuePriorities)) [[unlikely]]
            {
                ...
                LITEFX_WARNING(VULKAN_LOG, "No queues found at priority {queuePriority}. Attempting higher priority {nextPriority}.", priority, nextPriority);
            ...
            auto leastUsed = std::ranges::min_element(indices, {}, [](const auto& i) { return std::get<1>(i); });

            LITEFX_DEBUG(VULKAN_LOG, "Creating queue with id {queueId} (referenced: {refCount} times).", std::get<0>(*leastUsed), std::get<1>(*leastUsed));

            // Create a queue instance with the queue id.
            auto queue = makeUnique<VulkanQueue>(device, m_type, priority, m_id, std::get<0>(*leastUsed));
...
public:
    VulkanQueue* createQueue(QueueType type, QueuePriority priority)
    {
        ...
        // TODO: prefer queue family with least active queues.
        throw;
