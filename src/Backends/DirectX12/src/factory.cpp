...
UniquePtr<IDirectX12Image> DirectX12GraphicsFactory::createImage(const Format& format, const Size2d& size, const ImageDimensions& dimension, const UInt32& levels, const MultiSamplingLevel& samples) const
{
	...
	return DirectX12Image::allocate(m_impl->m_device, m_impl->m_allocator, size, format, dimension, D3D12_RESOURCE_STATE_ALL_SHADER_RESOURCE | D3D12_RESOURCE_STATE_UNORDERED_ACCESS, resourceDesc, allocationDesc);
}
...
UniquePtr<IDirectX12Image> DirectX12GraphicsFactory::createAttachment(const Format& format, const Size2d& size, const MultiSamplingLevel& samples) const
{
	...
	if (::hasDepth(format) || ::hasStencil(format))
	{
		...
		return DirectX12Image::allocate(m_impl->m_device, m_impl->m_allocator, size, format, ImageDimensions::DIM_2, D3D12_RESOURCE_STATE_ALL_SHADER_RESOURCE | D3D12_RESOURCE_STATE_DEPTH_READ, resourceDesc, allocationDesc);
	}
	else
	{
		resourceDesc.Flags = D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET;
		return DirectX12Image::allocate(m_impl->m_device, m_impl->m_allocator, size, format, ImageDimensions::DIM_2, D3D12_RESOURCE_STATE_ALL_SHADER_RESOURCE, resourceDesc, allocationDesc);
	}
}
...
UniquePtr<IDirectX12Texture> DirectX12GraphicsFactory::createTexture(const DirectX12DescriptorLayout& layout, const Format& format, const Size2d& size, const ImageDimensions& dimension, const UInt32& levels, const MultiSamplingLevel& samples) const
{
	D3D12_RESOURCE_DESC resourceDesc = {};
	resourceDesc.Dimension = ::getImageType(dimension);
	resourceDesc.Alignment = 0;
	...
	return DirectX12Texture::allocate(m_impl->m_device, layout, m_impl->m_allocator, size, format, dimension, levels, samples, D3D12_RESOURCE_STATE_COPY_DEST, resourceDesc, allocationDesc);
}

Array<UniquePtr<IDirectX12Texture>> DirectX12GraphicsFactory::createTextures(const DirectX12DescriptorLayout& layout, const UInt32& elements, const Format& format, const Size2d& size, const ImageDimensions& dimension, const UInt32& levels, const MultiSamplingLevel& samples) const
{
...
Array<UniquePtr<IDirectX12Sampler>> DirectX12GraphicsFactory::createSamplers(const DirectX12DescriptorLayout& layout, const UInt32& elements, const FilterMode& magFilter, const FilterMode& minFilter, const BorderMode& borderU, const BorderMode& borderV, const BorderMode& borderW, const MipMapMode& mipMapMode, const Float& mipMapBias, const Float& maxLod, const Float& minLod, const Float& anisotropy) const
{
