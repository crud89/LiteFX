...
private:
	/// @brief Stores temporary resource handles to render targets allocated during the frame buffer `resizing` event and returns them to the frame buffer in the allocation callback.
	SharedPtr<const IImage> m_depthBuffer, m_postColorBuffer;

public:
	/// @brief Initializes the overlapping resources.
	///
	/// @param renderArea The render area that determines the size of the resources.
