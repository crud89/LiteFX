...
void SampleApp::initPipelines()
{
    m_pipeline = m_renderPass->makePipeline(Pipelines::Basic, "Basic")
        ...
        .layout()
            .shaderProgram()
                .addVertexShaderModule("shaders/push_constants_vs.dxi")
                .addFragmentShaderModule("shaders/push_constants_ps.dxi")
                .go()
            .addDescriptorSet(DescriptorSets::Constant, ShaderStage::Vertex | ShaderStage::Fragment)
                .addUniform(0, sizeof(CameraBuffer))
                .go()
            .addPushConstants(sizeof(ObjectBuffer))
...
void SampleApp::drawFrame()
{
    ...
    // Bind both descriptor set for the camera buffer.
    ...
    for (int i(0); i < 9; ++i)
    {
        ...
        commandBuffer.pushConstants(*m_pipeline->layout().pushConstants(), &buffer);
