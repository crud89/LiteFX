...
void TestApp::onInit()
{
    ...
    auto startCallback = [](DirectX12Backend* backend) {
...

        {
            try
            {
                SharedPtr<IDirectX12VertexBuffer> buffer = factory.createVertexBuffer(layout, ResourceHeap::Resource, elements + 10u, ResourceUsage::Default, AllocationBehavior::DontExpandCache);
