...
void TestApp::onInit()
{
    ...
    auto startCallback = [](DirectX12Backend* backend) {
...

        {
            try
            {
                SharedPtr<IDirectX12Buffer> buffer = factory.createBuffer(BufferType::Uniform, ResourceHeap::Resource, defaultMaxPoolSize * 2, 1u, ResourceUsage::Default, AllocationBehavior::DontExpandCache);
