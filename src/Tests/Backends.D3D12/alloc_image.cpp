...
void TestApp::onInit()
{
    ...
    auto startCallback = [](DirectX12Backend* backend) {
        ...
        {
            SharedPtr<IDirectX12Image> image{};
...

        {
            try
            {
                SharedPtr<IDirectX12Image> image = factory.createTexture(Format::R32G32B32A32_SFLOAT, { 2048, 2048, 1 }, ImageDimensions::DIM_2, 1u, 1u, MultiSamplingLevel::x1, ResourceUsage::Default, AllocationBehavior::DontExpandCache);
...

        {
            try
            {
                SharedPtr<IDirectX12Image> image = factory.createTexture(Format::R32G32B32A32_SFLOAT, { 2048, 2048, 1 }, ImageDimensions::DIM_2, 1u, 1u, MultiSamplingLevel::x1, ResourceUsage::Default, AllocationBehavior::StayWithingBudget);
            }
            catch (RuntimeException& /*ex*/)
            {
