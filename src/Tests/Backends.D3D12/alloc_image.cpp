...
void TestApp::onInit()
{
    ...
    auto startCallback = [](DirectX12Backend* backend) {
...

        {
            try
            {
                SharedPtr<IDirectX12Image> image = factory.createTexture(Format::R32G32B32A32_SFLOAT, { 2048, 2048, 1 }, ImageDimensions::DIM_2, 1u, 1u, MultiSamplingLevel::x1, ResourceUsage::Default, AllocationBehavior::StayWithinBudget);
            }
            catch (RuntimeException& /*ex*/)
            {
