...
void TestApp::onInit()
{
    ...
    auto startCallback = [](DirectX12Backend* backend) {
        ...
        auto& layout = renderPipeline->layout()->descriptorSet(0);
        auto descriptorSets = layout.allocate(3) | std::ranges::to<std::vector>();

        if (descriptorSets.size() != 3)
            LITEFX_TEST_FAIL("descriptorSets.size() != 3");

        bool allSetsShareLayout = std::ranges::all_of(descriptorSets, [&layout](auto& set) { return std::addressof(set->layout()) == std::addressof(layout); });

        if (!allSetsShareLayout)
            LITEFX_TEST_FAIL("!allSetsShareLayout");

        // Drop first descriptor set in order to release it.
        descriptorSets = descriptorSets | std::views::as_rvalue | std::views::drop(1) | std::ranges::to<std::vector>();

        // Allocate more descriptor sets, to test mixed re-allocation.
        descriptorSets.append_range(layout.allocate(4));

        if (descriptorSets.size() != 6)
            LITEFX_TEST_FAIL("descriptorSets.size() != 6");
