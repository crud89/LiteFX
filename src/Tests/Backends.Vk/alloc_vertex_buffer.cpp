    ...
    auto startCallback = [](VulkanBackend* backend) {
        ...
        SharedPtr<VulkanInputAssembler> inputAssembler = _device->buildInputAssembler()
            ...
            .vertexBuffer(sizeof(Vertex), 0)
                ...
                .withAttribute(2, BufferFormat::XYZ32F, offsetof(Vertex, Normal), AttributeSemantic::Normal)
                .withAttribute(3, BufferFormat::XY32F, offsetof(Vertex, TextureCoordinate0), AttributeSemantic::TextureCoordinate, 0)
                .add();

        // Allocate some buffers.
        auto& layout = inputAssembler->vertexBufferLayout(0);
        ...
        static const UInt32 elements{ defaultMaxPoolSize / sizeof(Vertex) };

        // Attempt to allocate a large buffer with `NEVER_ALLOCATE`.
        {
            SharedPtr<IVulkanVertexBuffer> buffer{};

            if (factory.tryCreateVertexBuffer(buffer, layout, ResourceHeap::Resource, elements + 10u, ResourceUsage::Default, AllocationBehavior::DontExpandCache))
                LITEFX_TEST_FAIL("tryCreateVertexBuffer succeeded where it shouldn't.");
        }

        {
            try
            {
                SharedPtr<IVulkanVertexBuffer> buffer = factory.createVertexBuffer(layout, ResourceHeap::Resource, elements + 10u, ResourceUsage::Default, AllocationBehavior::DontExpandCache);
                LITEFX_TEST_FAIL("createVertexBuffer succeeded where it shouldn't.");
            }
...

        {
            try
            {
                SharedPtr<IVulkanVertexBuffer> buffer = factory.createVertexBuffer(layout, ResourceHeap::Resource, elements + 10u, ResourceUsage::Default, AllocationBehavior::StayWithingBudget);
            }
            catch (RuntimeException& /*ex*/)
            {
                LITEFX_TEST_FAIL("createVertexBuffer failed where it shouldn't.");
            }
        }
