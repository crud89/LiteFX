...
void TestApp::onInit()
{
    ...
    auto startCallback = [](VulkanBackend* backend) {
        ...
        // Create the device.
        _device = backend->createDevice("Default", *adapter, std::move(surface), Format::B8G8R8A8_UNORM, _viewport->getRectangle().extent(), 3, true, GraphicsDeviceFeatures { .RayTracing = true }).shared_from_this();
