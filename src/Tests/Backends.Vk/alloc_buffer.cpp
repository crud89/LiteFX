...
void TestApp::onInit()
{
    ...
    auto startCallback = [](VulkanBackend* backend) {
        ...
        {
            SharedPtr<IVulkanBuffer> buffer {};

            if (factory.tryCreateBuffer(buffer, BufferType::Uniform, ResourceHeap::Resource, defaultMaxPoolSize * 2, 1u, ResourceUsage::Default, AllocationBehavior::DontExpandCache))
                LITEFX_TEST_FAIL("tryCreateBuffer succeeded where it shouldn't.");
        }

        {
            try
            {
                SharedPtr<IVulkanBuffer> buffer = factory.createBuffer(BufferType::Uniform, ResourceHeap::Resource, defaultMaxPoolSize * 2, 1u, ResourceUsage::Default, AllocationBehavior::DontExpandCache);
                LITEFX_TEST_FAIL("createBuffer succeeded where it shouldn't.");
            }
...

        {
            try
            {
                SharedPtr<IVulkanBuffer> buffer = factory.createBuffer(BufferType::Uniform, ResourceHeap::Resource, defaultMaxPoolSize * 2, 1u, ResourceUsage::Default, AllocationBehavior::StayWithingBudget);
            }
            catch (RuntimeException& /*ex*/)
            {
                LITEFX_TEST_FAIL("createBuffer failed where it shouldn't.");
            }
        }
