...
void TestApp::onInit()
{
    ...
    auto startCallback = [](VulkanBackend* backend) {
        ...
        {
            SharedPtr<IVulkanIndexBuffer> buffer{};
...

        {
            try
            {
                SharedPtr<IVulkanIndexBuffer> buffer = factory.createIndexBuffer(layout, ResourceHeap::Resource, elements + 10u, ResourceUsage::Default, AllocationBehavior::DontExpandCache);
...

        {
            try
            {
                SharedPtr<IVulkanIndexBuffer> buffer = factory.createIndexBuffer(layout, ResourceHeap::Resource, elements + 10u, ResourceUsage::Default, AllocationBehavior::StayWithingBudget);
            }
            catch (RuntimeException& /*ex*/)
            {
