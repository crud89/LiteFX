...
void TestApp::onInit()
{
    ...
    auto startCallback = [](VulkanBackend* backend) {
        ...
        SharedPtr<VulkanShaderProgram> shaderProgram = _device->buildShaderProgram()
            ...
            .withFragmentShaderModule("shaders/test_cr_fs.spv");
        ...
        if (std::as_const(*renderPipeline->rasterizer()).conservativeRasterization() != true)
            LITEFX_TEST_FAIL("renderPipeline->rasterizer()->conservativeRasterization() != true");
