...
private:
    UInt32 m_location, m_offset;
    BufferFormat m_format;

public:
    BufferAttributeImpl(BufferAttribute* parent, const UInt32& location = 0, const UInt32& offset = 0, const BufferFormat& format = BufferFormat::None) :
        base(parent), m_location(location), m_offset(offset), m_format(format) { }
};
...
BufferAttribute::BufferAttribute(const UInt32& location, const UInt32& offset, const BufferFormat& format) :
    m_impl(makePimpl<BufferAttributeImpl>(this, location, offset, format))
{
}
